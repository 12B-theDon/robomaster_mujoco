...
class Arm(Module):
    ...
    def __init__(self, robot: robomaster.robot.Robot, node: 'RoboMasterROS') -> None:
        ...
        # Joystick-friendly arm velocity teleop (m/s on x/z) via topic `cmd_arm_vel`.
        # This is separate from the legacy `cmd_arm` delta interface.
        self._cmd_arm_vel_action: Optional[robomaster.action.Action] = None
        self._cmd_arm_vel_x_m_s: float = 0.0
        self._cmd_arm_vel_z_m_s: float = 0.0
        self._cmd_arm_vel_target_x: Optional[int] = None  # mm
        self._cmd_arm_vel_target_z: Optional[int] = None  # mm
        self._cmd_arm_vel_last_tick_ns: Optional[int] = None
        self._cmd_arm_vel_last_msg_ns: Optional[int] = None
        cmd_arm_vel_rate_hz = float(node.declare_parameter('arm.cmd_arm_vel.rate', 20.0).value)
        cmd_arm_vel_rate_hz = max(1.0, cmd_arm_vel_rate_hz)
        self._cmd_arm_vel_timeout_s = float(node.declare_parameter('arm.cmd_arm_vel.timeout', 0.25).value)
        self._cmd_arm_vel_deadband_m_s = float(node.declare_parameter('arm.cmd_arm_vel.deadband', 0.01).value)
        self._cmd_arm_vel_max_step_mm = int(node.declare_parameter('arm.cmd_arm_vel.max_step_mm', 8).value)
        self._cmd_arm_vel_abort_on_stop = bool(node.declare_parameter('arm.cmd_arm_vel.abort_on_stop', True).value)

        node.create_subscription(geometry_msgs.msg.Vector3, 'cmd_arm',
                                 self.has_received_velocity_command, 10)
        node.create_subscription(geometry_msgs.msg.Vector3, 'cmd_arm_vel',
                                 self.has_received_velocity_command_vel, 10)
        ...
        node.create_timer(1.0 / cmd_arm_vel_rate_hz, self._cmd_arm_vel_tick)
    ...
    def has_received_velocity_command_vel(self, msg: geometry_msgs.msg.Vector3) -> None:
        # Velocity command in m/s on x/z. Intended for joystick teleop (continuous axes).
        self._cmd_arm_vel_x_m_s = float(msg.x)
        self._cmd_arm_vel_z_m_s = float(msg.z)
        self._cmd_arm_vel_last_msg_ns = int(self.clock.now().nanoseconds)

    def _abort_cmd_arm_vel_action(self) -> None:
        if not self._cmd_arm_vel_action:
            return
        dispatcher = self.robot.action_dispatcher
        msg = dispatcher.get_msg_by_action(self._cmd_arm_vel_action)
        msg._proto._action_ctrl = 1
        self.robot.client.send_msg(msg)
        self._cmd_arm_vel_action = None

    def _cmd_arm_vel_tick(self) -> None:
        # Do not interfere with explicit MoveArm actions.
        if self.action:
            return
        if not self.node.connected:
            return

        now_ns = int(self.clock.now().nanoseconds)
        if self._cmd_arm_vel_last_tick_ns is None:
            self._cmd_arm_vel_last_tick_ns = now_ns
            return
        dt_s = max(0.0, (now_ns - self._cmd_arm_vel_last_tick_ns) * 1e-9)
        self._cmd_arm_vel_last_tick_ns = now_ns

        stale = (
            self._cmd_arm_vel_last_msg_ns is None
            or (now_ns - int(self._cmd_arm_vel_last_msg_ns)) > int(self._cmd_arm_vel_timeout_s * 1e9)
        )
        vx = 0.0 if stale else float(self._cmd_arm_vel_x_m_s)
        vz = 0.0 if stale else float(self._cmd_arm_vel_z_m_s)

        if abs(vx) < self._cmd_arm_vel_deadband_m_s:
            vx = 0.0
        if abs(vz) < self._cmd_arm_vel_deadband_m_s:
            vz = 0.0

        if vx == 0.0 and vz == 0.0:
            if self._cmd_arm_vel_abort_on_stop:
                self._abort_cmd_arm_vel_action()
            return

        if self._cmd_arm_vel_target_x is None or self._cmd_arm_vel_target_z is None:
            self._cmd_arm_vel_target_x = int(getattr(self, "x", 0))
            self._cmd_arm_vel_target_z = int(getattr(self, "z", 0))

        dx_mm = int(round(vx * 1000.0 * dt_s))
        dz_mm = int(round(vz * 1000.0 * dt_s))
        if self._cmd_arm_vel_max_step_mm > 0:
            dx_mm = max(-self._cmd_arm_vel_max_step_mm, min(self._cmd_arm_vel_max_step_mm, dx_mm))
            dz_mm = max(-self._cmd_arm_vel_max_step_mm, min(self._cmd_arm_vel_max_step_mm, dz_mm))
        if dx_mm == 0 and dz_mm == 0:
            return

        self._cmd_arm_vel_target_x += dx_mm
        self._cmd_arm_vel_target_z += dz_mm

        # Abort previous teleop action to avoid action backlog (non-blocking).
        self._abort_cmd_arm_vel_action()

        # Send a new absolute target. SDK uses (x, y) for arm plane; we map ROS z -> SDK y.
        while True:
            try:
                new_action = self.api.moveto(
                    x=int(self._cmd_arm_vel_target_x), y=int(self._cmd_arm_vel_target_z)
                )
                break
            except RuntimeError as e:
                self.logger.warning(f'[Arm] cmd_arm_vel: Failed creating new action {e} => remove current action')
                remove_action_with_target(
                    self.robot, robomaster.robotic_arm.RoboticArmMoveAction._target)
        self._cmd_arm_vel_action = new_action
